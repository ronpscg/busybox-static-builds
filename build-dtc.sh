...
# They are not really needed. I thought I would build directly from the kernel tree at first.
declare -A ARCHS
...
build_for_several_tuples() {
	...
	for tuple in $TUPLES $MORE_TUPLES ; do
		...
		# export ARCH=${ARCHS[$tuple]} # this is not needed. see comment above
...
fetch() (
	[ "$1" = "dontfetch" ] && return
	# the last time this was updated, master was at commit 9197f1ccd95c9475006677aa97bf39727c1e8aa5 (sometime after v1.7.2)
